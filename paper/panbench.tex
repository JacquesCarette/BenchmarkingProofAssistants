...
\supplement{}
\supplementdetails[linktext={panbench}, cite=Mullanix:2026]{Software}{https://github.com/JacquesCarette/panbench}
...
syntactic structure. To fill this gap, we have created the \panbench~\cite{Mullanix:2026} system, which consists of:
...
basic infrastructure improvements. For instance, we do not consider a file
containing only \(10^{7}\) newlines to be a realistic input to any of the
systems in question. However, being able to lex and parse large inputs \emph{is}
something that we expect of these systems, doubly so if we want to be able to
typecheck proofs that have been generated by an external tool.
...
const5 : {A : Type} -> {B0, B1, B2, B3, B4 : Type} ->
  A -> B0 -> B1 -> B2 -> B3 -> B4 -> A
...
We ran all systems in their default configuration. The versions of
the systems we used are listed in following table.

\begin{figure}[htb]
  \centering
  \begin{tabular}{l|l}
    \textbf{System} & \textbf{Version} \\ \hline
    \agda & v2.8.0 \\
    \idris & v0.7.0 \\
    \lean & v4.21.0 \\
    \rocq & v9.0.0 \\
  \end{tabular}
\caption{System versions used in our benchmarks.}
\label{tab:versions}
\end{figure}

We also made the choice to run all systems in their default configuration when
possible, as this is what the average user would experience. Similarly, we have
opted to include start-up time in our plots: a 1 second startup time in batch
mode is something worth investigating!\footnote{All the systems we tested do have an interactive mode, which does make
  startup time less relevant to the user experience. However, benchmarking interactive modes
  requires \emph{significantly} more engineering effort, so we have opted to
  only consider batch mode.
}.
...
slim (64 MB) in \agda to a 29 times slower in \idris and 4.8 times memory consumer in \lean.
...
This might seem somewhat contrived on its face. However, a reviewer helpfully pointed out that
modern CPU instruction sets have thousands of distinct opcodes, so this is well within scope
of what these tools are expected to handle.
...
Figure~\ref{fig:IdChain}, corresponding to Listing~\ref{code:IdChain} tests nested calls to
the identity function. This shows even more extreme behaviour: basically instantaneous until
memory explosion and time out. This is well-known to be a problem for type systems based on
Hindley-Milner inference, but it is less clear that it ought to be a weakness for bidirectional
typing as well. Closer sampling (not shown) does not change this picture.

\begin{figure}[h]
\begin{minipage}{0.50\textwidth}
\scalebox{0.75}{\input{pgfs/NestedLetXor/user}}
\end{minipage}
\begin{minipage}{0.50\textwidth}
\scalebox{0.75}{\input{pgfs/NestedLetXor/rss}}
\end{minipage}
\caption{Nested let bindings, simple addition on rhs}
\label{fig:NestedLetAdd}
\end{figure}

Figure~\ref{fig:NestedLetAdd}, corresponding to Listing~\ref{code:LetAdd}, tests a series
of nested let bindings doing very simple arithmetic. The contrast is remarkable: \agda and \idris time out already
...
The previous section analyzed the results themselves. The following section
contains some higher-level conclusions, along with some root-cause analyses
of some of the performance issues we found.

\subsection{\agda}

Most of the results for \agda follow a general pattern: it is consistently the
...
by the use of Haskell's \lstinline|String| inside of Agda's lexer. This is a
known performance pitfall: a \lstinline|String| is a lazy linked list of lazy
unicode codepoints, which is suboptimal along basically all axes.
...
of any systems we benchmarked, coming in at over half a second. This is a side effect of using
\chez~\cite{Dybvig:2006} as a runtime, which incurs a high interpreter startup cost.
...
and consumes over 6 gigabytes while doing so. The culprit is \idris's \texttt{Text.Lexer} library,
which is implemented as a tree-walking regular expression interpreter. Through private conversations
the developers, we learned that this was done to avoid having to mark the lexer as \texttt{partial}.

It also struggles with large numbers of binders, and times out when trying to elaborate a linear sequence of 1024
let bindings or \(\lambda\) abstractions. After some investigation, we observed that \idris weakens the entire typechecking state
when going underneath a binder, and then subsequently strengthens it when it leaves. This results in
a quadtratic blowup when it needs to traverse under large numbers of binders.

However, there are some bright spots. \idris performs \emph{exceptionally} well on tests
involving elaboration of large numbers of datatypes and records once the startup time is accounted for.
...
on elaboration tasks that involve large numbers of names. Private communication
with the developers revealed that this is a result of using singly-linked lists
for maintaining names introduced via \texttt{open} statements, which can lead
to quadratic behaviour during name resolution due to repeated \(\mathcal{O}(n)\)
lookups.

On the bright side, \lean does manage to elaborate the nested addition test,
though it does seem to exhibit some non-linear behaviour while doing so. Through
private communication with the \lean developers, we learned that this is due to
quadratic behaviour during instance resolution. When presented with a
chain of let definitions \texttt{x_{i+1} := x_{i} + x_{i}}, \lean creates a
tyepclass metavariable \texttt{HAdd ?\(\alpha_{i}\) ?\(\alpha_{i}\) ?\(\alpha_{i+1}\)}, and then
proceeds to resolve these metavariables inside out. Due to the dependent nature of the
metavariables, \lean is only able to solve them one at a time. However, \lean will retry
\emph{all} of the pending metavariables on each synthesis step, resulting in a quadratic
blow up.
...
determines the output. On the surface, this is a reasonable ask, but taking this
...
shows that their encoding of the judgements actually aligns with the language. Typically,
such proofs are done on paper. However, mechanizing this adequacy proof would require a
staged third layer consisting of a more traditional proof assistant, where one could
directly encode the structure of the langauge in question and perform the usual
induction over canonical forms in the logical framework.
...
four target grammars differ significantly in how they treat type signatures.
Those that prioritize dependent pattern matching (e.g. \agda, \idris) typically
opt to have standalone type signatures: this allows for top-level pattern
matches, which ensures that the unification problems faced during pattern matching lie
within a tractable fragment\footnote{This approach is also taken by the \texttt{Equations}~\cite{Sozeau:2010} plugin for \rocq.}~\cite{Coquand:1992}.
Conversely, languages oriented around tactics (e.g. \lean, \rocq) typically opt for in-line type signatures and
...
As an example, Listing~\ref{code:IdChain} looks as shown below in \panbench. We elide
