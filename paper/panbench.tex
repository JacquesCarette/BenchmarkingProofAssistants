...
\begin{enumerate}
\item concrete benchmarks and analysis thereof,
\item the supporting extensible infrastructure, and
\item the design of the embedded \panbench DSL for system-agnostic tests.
\end{enumerate}
All three required extensive work, including many design iterations, before settling on
what is here.

The rest of this paper is structured as follows. We detail our methodology
...
Conversion & Addition        &         & \\
\end{tabular}
\caption{A breakdown of the \panbench benchmark suite.}
\label{tab:tests}
\end{figure}
...
only system that was able to handle checking 32 iterated applications of the identity
function, and did so with ease, taking approximately 0.25 seconds and a negligible
...
\item It must allow for multiple revisions of the same tool to be installed
  simultaneously.
\item It must allow for multiple copies of the \emph{same} version of a tool to be installed
...
\item Nix does not work natively on Windows. Performance problems
  are often operating system specific, so ruling out an OS that has
