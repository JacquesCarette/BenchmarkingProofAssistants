...
and inference rules. In this sense \panbench is a \emph{grammatical} framework\footnote{Not to be confused with \emph{the} Grammatical Framework~\cite{Ranta:2011}, which aims to be a more general meta-framework for implementing and translating between grammars.} rather than a logical
one.
...
judgements. Then, one needs to prove an proving an adequacy theorem on the side that
...
the grammar of non-binding terms, though their treatment of binding forms and top level definitions diverges.
...
as \emph{abstract} syntax, they may have different performance characteristics, so so we cannot simply lower them to a
...
\end{lstlisting}
Production rules that involve binding forms as classes that are parametric
over a notion of a binding cell, as demonstrated below.
\begin{lstlisting}[style=haskell, caption=]
-- | Pi types.
...
This lets us decouple the grammatical concept of a binding forms like \(\Pi\)-types or
\(\lambda\)-abstractions from the particularities of a given languages bindings.
...
This is a case where the granular tagless encoding shines: both \lean and \rocq support semi-implicits/non-maximal implicits;
IE: implicit arguments that are not eagerly instantiated. On the other hand, \idris and \agda have no such notion. Thus, by
decomposing the language of binding modifiers into small pieces, we can easily write generators that target only the languages
...
Unfortunately, we are not out of the woods yet: like binders, there are a plethora of different
top-level definition forms. The key insight we had was that top level definitions \emph{are binding forms},
and that both should share the same underlying language. This perspective lets us radically simplify how
we view some more baroque top-level bindings; as a contrived example, consider the following \agda
definition:

\todo[inline]{Reed: unicode :(}
\begin{lstlisting}[style=agda]
...
In the our language of binders, this definition is a \(2\)-ary annotated binder
that

% \begin{lstlisting}[caption=Selected Panbench classes]
% class Binder arity nm ann tm cell | cell -> nm tm where
%   binder :: arity nm -> ann tm -> cell
