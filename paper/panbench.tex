...
syntax across multiple different langauges. Crucially, \panbench does \emph{not}
...
\begin{enumerate}
\item A layer for defining judgements ala relations.
...
If we take this basic skeleton of a design and transpose it to work with grammatical constructs
rather than logical ones, we will also obtain three layers:

\begin{enumerate}
\item A layer for defining grammars ala relations.
\item A logic programming layer for synthesizing derivations.
...
This style of language encoding is typically known as \emph{finally tagless}\cite{Carette-Kiselyov-Shan:2009},
...
this more fine-grained approach in later sections.
...
proof assistant caries a lot more information than just a variable name and a type. After a careful study
of our four intended target languages, we classified the various binding features along 3 separate axes:
binding arity, binding annotations, and binding modifiers.

Binding arities and annotations relatively self-explanatory, and classify the number of names bound, along with
the type of annotation allowed. After studying our four target languages, we noticed that all binding
arities fell into one of three classes: \(n\)-ary, unary, or nullary. We can similarly characterise annotations
into three categories: required, optional, or forbidden.

On the other hand, binding modifiers require a bit more explanation. A binding modifier capture features like
...
or quantities in Idris 2 behave similarly: they do not change the shape of the binding. Currently,
...
The question of top-level defintions is much thornier, and there seems to be less agreement on how
...
Conversely, languages oriented around tactics typically opt for in-line type signatures, which
leads to more compact definitions.

This presents us with a design decision: should our idealized grammar use inline or standalone signatures?
We have opted for the former, as standalone signatures offer more degrees of freedom that languages in-line
...
distinction. To accound for this, we were forced to refine the visibility modifier system to distinguish
