...
just a few that should be \emph{illustrative} of the rest. The source of
...
\begin{lstlisting}[style=agda,caption="Nesting: Id chain (Agda)"]
...
\end{minipage}
\begin{minipage}{0.49\textwidth}
\begin{lstlisting}[style=lean,caption="Nesting: Let add (Lean)"]
def n : Nat :=
  let x0 := 1
  let x1 := x0 + x0
  let x2 := x1 + x1
...
\begin{lstlisting}[style=idris,caption="Binders: Lambda (Idris 2)"]
module Main
...
\begin{lstlisting}[style=rocq,caption="Dependency: Record chains (Rocq)"]
